...
def test_turns_are_successfully_created(init_database, test_client, user1_username, user2_username, match):
    match.started = True
...
def test_turns_returns_400_if_match_doesnt_exist(init_database, test_client, user1_username, user2_username):
    db.session.commit()
...
def test_turns_returns_400_if_match_has_not_started(
        init_database, test_client, user1_username, user2_username, match, turn):
    db.session.add(match)
    db.session.add(turn)
    db.session.commit()
...
def test_turns_returns_400_if_match_has_finished(
        init_database, test_client, user1_username, user2_username, match, turn):
    match.finished = True
    db.session.add(match)
    db.session.add(turn)
